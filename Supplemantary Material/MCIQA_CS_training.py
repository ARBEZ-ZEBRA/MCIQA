...
class ICQADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        prompt = "<|image_pad|>Analyze the visual quality and density distribution of this image." 
...
def main():
    ...
    train_json = 'MCIQA_2K_CS_MOS_train.json'
    test_img_dir = 'MCIQA_2K_test'
    test_json = 'MCIQA_2K_CS_MOS_test.json'
    train_weight_dir = '/MCIQA_2K_CS_MASK'
    test_weight_dir = '/MCIQA_2K_CS_MASK'
    train_result_json = 'MCIQA_2K_CS_MOS_train.json'
    test_result_json = 'MCIQA_2K_CS_MOS_test.json'
    batch_size = 4  
    epochs = 50
    ...
    # state_dict = torch.load("best_cs_qwen.pth", map_location='cpu')
    ...
    for epoch in range(epochs):
        ...
        if 0.9*avg_train_loss + 0.1*avg_test_loss < best_loss:
            best_loss = 0.9*avg_train_loss + 0.1*avg_test_loss
            torch.save(model.state_dict(), "best_cs_qwen.pth")
            print("Model saved!")

    torch.save(model.state_dict(), "final_cs_qwen.pth")
    print("Model saved!")
